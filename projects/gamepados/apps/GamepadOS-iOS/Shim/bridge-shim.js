...
(function () {
  ...
  var state = {
    ...
    // Seeded from the boot script injected BEFORE this shim (see
    // MainViewController.setupWebView) so the UI's mount-time version read
    // never sees 0 — that reads as "update available". Native push refreshes.
    versionName: (window.__iosBootVersion && window.__iosBootVersion.name) || "0.0.0",
    versionCode: (window.__iosBootVersion && window.__iosBootVersion.code) || 0
  };
  ...
  window.AndroidBridge = {
    // ── Platform marker: lets the UI hide Android-only surfaces (USB/ADB
    //    transports, Wi-Fi radio info, battery exemptions). The Android JNI
    //    bridge has no getPlatform, so absence = android. ──
