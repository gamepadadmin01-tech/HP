...
final class MotionEngine {
    ...
    init() {
        ...
        // Device-orientation notifications are DEAD unless generation is on —
        // without this call the landscape flag froze at its default and one of
        // the two landscape holds steered inverted.
    ...
    /// Authoritative mapping from the INTERFACE orientation — what Android's
    /// display-rotation remap keys on. The app locks orientation via geometry
    /// updates, so the interface can differ from the physical pose; interface
    /// .landscapeRight corresponds to device .landscapeLeft.
    func setInterfaceOrientation(_ io: UIInterfaceOrientation) {
        switch io {
        case .landscapeRight: setLandscape(left: true)
        case .landscapeLeft: setLandscape(left: false)
        default: break
        }
    }
    ...
    func start() {
        ...
        // SENSOR_DELAY_FASTEST parity. Request above the hardware ceiling —
        // CoreMotion clamps to the max fusion rate, so every push carries the
        // freshest sample the device can produce.
        manager.deviceMotionUpdateInterval = 1.0 / 400.0
