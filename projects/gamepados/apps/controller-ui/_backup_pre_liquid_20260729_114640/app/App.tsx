        ...
        <div className="flex-1 overflow-hidden">
          ...
          <div ref={gyroTilt.leftBarRef} className="h-full w-full"
               style={{ transform: "scaleX(0)", transformOrigin: "right center", willChange: "transform",
                        background: "#4F86C6", boxShadow: "0 0 8px rgba(79,134,198,0.9)" }} />
        </div>
        <div className="flex-1 overflow-hidden">
      ...
      <div className="absolute z-10 flex flex-col items-center pointer-events-none w-full"
        ...
        <div className="flex items-center justify-center gap-2 px-4 w-full">
          ...
          <button data-nx="1"
            ...
            style={{
              color: gyroOn ? "#4F86C6" : "rgba(255,255,255,0.45)",
            ...
            <span style={{
              ...
              background: gyroOn ? "#4F86C6" : "rgba(255,255,255,0.3)",
              boxShadow: gyroOn ? "0 0 5px #4F86C6" : "none"
            }} />
            GYRO
          </button>
...
function ScannerScreen({ onDashboard }: { onDashboard: () => void }) {
  ...
  const stepDot = (n: number, active: boolean) => (
    <div className="flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold"
      style={{ background: active ? "rgba(79,134,198,0.15)" : "rgba(255,255,255,0.05)", border: `1.5px solid ${active ? "#4F86C6" : "rgba(255,255,255,0.1)"}`, color: active ? "#4F86C6" : "rgba(255,255,255,0.35)" }}>
      {n}
    </div>
  );
        ...
        <div className="flex gap-2 p-1 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
          {(["wireless", "wired"] as const).map(t => (
            <button key={t} onClick={() => { setTab(t); setShowManual(false); }}
              ...
              style={tab === t
                ? { background: t === "wireless" ? "rgba(79,134,198,0.15)" : "rgba(90,16,16,0.5)", color: t === "wireless" ? "#4F86C6" : "#ff6060", border: `1px solid ${t === "wireless" ? "rgba(79,134,198,0.3)" : "rgba(200,40,40,0.4)"}` }
            ...
            >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <button
                      ...
                      style={{ background: "transparent", border: "1px solid rgba(79,134,198,0.35)", color: "#4F86C6" }}>
                      Get Download Link
                    </button>
                  </div>
                </div>
              ...
              <div className="rounded-2xl p-4" style={{ background: "rgba(79,134,198,0.04)", border: "1px solid rgba(79,134,198,0.15)" }}>
                ...
                <button onClick={() => setShowScanner(true)}
                  ...
                  style={{ background: "rgba(79,134,198,0.18)", border: "1.5px solid rgba(79,134,198,0.4)", color: "#4F86C6" }}>
                  📷 Scan PC QR Code
                </button>
                ...
                {showManual && (
                  <div className="mt-1 space-y-2">
                    ...
                    <button onClick={handleManualConnect}
                      ...
                      style={{ background: "rgba(79,134,198,0.12)", border: "1px solid rgba(79,134,198,0.3)", color: "#4F86C6" }}>
                      Connect
                    </button>
                  </div>
                )}
...
                }}
                  className="w-full accent-[#4F86C6] h-1 rounded-lg appearance-none cursor-pointer bg-white/10" />
              </div>
...
            </div>
            ...
            <div>
              ...
              <button onClick={() => setGyroMode("racing")}
                ...
                style={gyroMode === "racing" ? { background: "rgba(79,134,198,0.18)", color: "#4F86C6", border: "1px solid rgba(79,134,198,0.5)" } : { background: "rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.6)", border: "1px solid rgba(255,255,255,0.12)" }}>
                Racing
              </button>
              <button onClick={() => setGyroMode("3d")}
                className="flex-1 px-3 py-2 rounded-lg text-sm font-semibold transition"
                style={gyroMode === "3d" ? { background: "rgba(79,134,198,0.18)", color: "#4F86C6", border: "1px solid rgba(79,134,198,0.5)" } : { background: "rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.6)", border: "1px solid rgba(255,255,255,0.12)" }}>
                3D
              </button>
              </div>
          ...
          {state === "ok" ? (
            <div className="py-6 text-center space-y-2">
              ...
              <button onClick={closeTray} className="mt-2 px-5 py-2 rounded-lg text-[13px] font-semibold"
                style={{ background: "rgba(79,134,198,0.15)", color: "#4F86C6", border: "1px solid rgba(79,134,198,0.35)" }}>Done</button>
            </div>
          ) : (
            <div className="space-y-3">
              ...
              <button disabled={!valid || state === "sending"} onClick={submit}
                ...
                style={{
                  ...
                  color: valid ? "#4F86C6" : "rgba(255,255,255,0.4)",
  ...
  if (compact) {
    ...
    return (
      <div className="mb-3 rounded-xl p-3 flex items-center justify-between gap-3"
        ...
        {busy ? (
          <span className="flex-shrink-0 text-[11px] font-bold" style={{ color: "#4F86C6" }}>{dlText}</span>
        ) : (
          <button onClick={download}
            className="flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-bold active:scale-95 transition-transform"
            style={{ background: "rgba(79,134,198,0.18)", border: "1px solid rgba(79,134,198,0.4)", color: "#4F86C6" }}>
            {dl ? "Retry" : "Update"}
          </button>
        )}
      </div>
    ...
    <div className="p-4 space-y-2">
      {state === "available" ? (
        <div className="space-y-2">
          <p className="text-xs font-semibold" style={{ color: "#4F86C6" }}>Update available{info.version ? `: v${info.version}` : ""}</p>
          {info.notes && <p className="text-[11px] text-muted-foreground leading-relaxed">{info.notes}</p>}
          {busy ? (
            <div className="space-y-1">
              <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.1)" }}>
                <div className="h-full transition-all" style={{ width: `${Math.max(3, dl?.pct ?? 0)}%`, background: "#4F86C6" }} />
              </div>
              <p className="text-[11px]" style={{ color: "#4F86C6" }}>{dlText}</p>
            </div>
          ) : (
            <>
              {dl && (dl.phase === "error" || dl.phase === "permission") && (
                <p className="text-[11px]" style={{ color: dl.phase === "error" ? "#ff6b6b" : "#4F86C6" }}>{dl.msg}</p>
              )}
              <button onClick={download}
                className="w-full py-2 rounded-lg text-xs font-bold active:scale-95 transition-transform"
                style={{ background: "rgba(79,134,198,0.15)", border: "1px solid rgba(79,134,198,0.35)", color: "#4F86C6" }}>
             ...
             reachedServer.current
               ? <>Our update service isn't responding. Get the latest version at{" "}
                   <a href="https://gamepad.space/#download" target="_blank" rel="noreferrer"
                      className="underline" style={{ color: "#4F86C6" }}>gamepad.space</a>.</>
      ...
      <header className="flex-shrink-0 flex items-center justify-between px-4 pb-3 border-b border-border bg-[#10141C]"
        ...
        >
          <span className="w-5 h-5 rounded-full flex items-center justify-center"
            style={{
              background: session ? "#4F86C6" : "rgba(255,255,255,0.10)",
