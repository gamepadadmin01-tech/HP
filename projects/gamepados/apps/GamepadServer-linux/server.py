...
def _acquire_single_instance():
    """Ensure only ONE server runs per user session. Windows: named mutex
    (auto-released on process exit). POSIX: flock on a per-user lock file
    (auto-released on process exit). Returns a handle to keep alive for the
    process lifetime, True if the guard can't be created (never block
    startup), or None if another instance already owns it."""
    if not sys.platform.startswith("win"):
        try:
            ...
            lock_path = os.path.join(
                tempfile.gettempdir(),
                "gamepadserver-%s.lock" % os.environ.get("USER", "user"))
            f = open(lock_path, "w")
            try:
                ...
                f.close()
                return None
            return f  # keep the fd (and thus the lock) alive
        except Exception:
            return True
    try:
        import ctypes
...
def _load_vgamepad():
    if not sys.platform.startswith("win"):
        # Linux: kernel uinput backend (virtualpad_linux) — games see a real
        # X360 pad, rumble flows back via force feedback. Other POSIX (macOS
        # dev boxes): mock pad so the network/QR layers still run.
        if sys.platform.startswith("linux"):
            return importlib.import_module("virtualpad_linux")
        return importlib.import_module("virtualpad_mock")
    try:
...
def download_update(url, dest_path, progress_cb=None, expected_sha256=None, timeout=30):
...
def _posix_ipv4_adapters():
    """[(interface name, ipv4), ...] via libc getifaddrs — Linux/macOS
    equivalent of _win_ipv4_adapters ([] on Windows or any failure)."""
    if os.name == "nt":
        return []
    try:
        import ctypes
        ...
        class _ifaddrs(ctypes.Structure):
            pass
        # Common leading fields on Linux and the BSDs/macOS — we only read these.
        _ifaddrs._fields_ = [
        ...
        try:
            ...
            while cur:
                ...
                sa = a.ifa_addr
                if sa:
                    # sockaddr family: u16 at offset 0 on Linux; u8 at offset 1
                    # on macOS/BSD (sa_len comes first).
                    if sys.platform.startswith("linux"):
                        fam = ctypes.cast(sa, ctypes.POINTER(ctypes.c_ushort)).contents.value
                    else:
                        fam = ctypes.string_at(sa + 1, 1)[0]
                    if fam == socket.AF_INET:
                        raw = ctypes.string_at(sa, 8)      # sockaddr_in header
                        ip = socket.inet_ntoa(raw[4:8])    # sin_addr at offset 4
                        name = (a.ifa_name or b"").decode(errors="replace")
                        if ip and not ip.startswith("127."):
                            out.append((name, ip))
                cur = a.ifa_next
        finally:
...
def _ipv4_adapters():
    """Platform-appropriate [(description-or-ifname, ipv4), ...]."""
    return _win_ipv4_adapters() if os.name == "nt" else _posix_ipv4_adapters()
...
def _all_ipv4():
    ...
    try:
        ...
        pass
    for _desc, ip in _ipv4_adapters():
        if ip not in ips:
            ips.append(ip)
    return ips
...
# Linux: a phone's USB tether shows up as usb0/rndis0 or an enx<MAC> interface.
_TETHER_IFACE_RE = re.compile(r"^(usb\d+|rndis\d*|enx[0-9a-f]{12})$", re.I)
_tether_cache = {"t": 0.0, "subnets": []}

def _usb_tether_subnets():
    now = time.monotonic()
    if now - _tether_cache["t"] > 5.0:
        rx = _TETHER_DESC_RE.search if os.name == "nt" else _TETHER_IFACE_RE.match
        _tether_cache["subnets"] = [
            ip.rsplit(".", 1)[0]
            for desc, ip in _ipv4_adapters() if rx(desc or "")
        ]
        _tether_cache["t"] = now
...
def _gp_config_dir():
    """Per-user config dir (%LOCALAPPDATA%\\GamepadServer on Windows,
    ~/.config/GamepadServer elsewhere). Holds the pairing key and the first-run
    setup marker. Created lazily; falls back to the home dir."""
    if os.name == "nt":
...
def boost_process_priority():
    """Raise this process's scheduling priority so the UDP receive thread isn't
    delayed under load. Windows: HIGH priority class. POSIX: negative nice
    (silently requires privileges). Best-effort; ignored if it fails."""
    if os.name != "nt":
        try:
            os.nice(-5)
        except Exception:
...
class PadManager:
    ...
    def _free(self, s):
        ...
        try:
            ...
            pass
        # Non-ViGEm backends (Linux uinput / mock) expose close(): destroy the
        # kernel device NOW — same "no ghost pad" guarantee as the unregister
        # above (the uinput FF reader thread would otherwise keep the pad alive).
        try:
            if hasattr(pad, "close"):
                with GAMEPAD_LOCK:
                    pad.close()
        except Exception:
            pass
...
def apply_inputs(sess, buttons, lt, rt, ls_x, ls_y, rs_x, rs_y):
    ...
    def _snap(v):
        return 128 if abs(v - 128) <= 4 else v
    ls_x = _snap(ls_x); ls_y = _snap(ls_y)
    rs_x = _snap(rs_x); rs_y = _snap(rs_y)
    ...
    with GAMEPAD_LOCK:
        ...
        def _axis(b):
            return max(-1.0, min(1.0, (b - 128) / 127.0))
        gamepad.left_joystick_float(x_value_float=_axis(ls_x), y_value_float=-_axis(ls_y))
        gamepad.right_joystick_float(x_value_float=_axis(rs_x), y_value_float=-_axis(rs_y))
...
_NO_WINDOW = 0x08000000 if os.name == "nt" else 0  # CREATE_NO_WINDOW — keep adb console hidden in the windowed app (creationflags must be 0 on POSIX)
    ...
    def _install_worker():
        ...
        def after():
            ...
            if not launch_installer_elevated(dest):
                ...
                return
            # Quit immediately so the installer can replace our files (mutex frees on exit).
            os._exit(0)
...
def main():
    ...
    def _load_or_create_key():
        kdir = _gp_config_dir()   # LOCALAPPDATA\GamepadServer / ~/.config/GamepadServer
