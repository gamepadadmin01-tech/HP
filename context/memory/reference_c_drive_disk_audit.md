...
metadata: 
  ...
  modified: 2026-08-04T04:48:16.793Z
---
...
## Reclaimed 2026-08-04 (17.6 → 29.7 GB free, +12.1)
npm `_cacache` + `_npx` + Yarn cache 2.22, Chrome `OptGuideOnDeviceModel` 3.98 (back again — it
re-downloads, so it is reclaimable on every pass), user Temp 0.89, `.gemini\antigravity*` 4.95
(three folders: antigravity / -ide / -backup; IDE state only, not source).

## 🚨 BUILD CACHES ARE NOW ON D: — keep them there
`npm config set cache "D:\AKHIL\.caches\npm"` and `cache-folder "D:\\AKHIL\\.caches\\yarn"` in
`~/.yarnrc`. **Yarn 1 ignores `npm_config_cache`** — it has its own cache at
`%LOCALAPPDATA%\Yarn\Cache`, which is how a `npx yarn install` silently put 0.44 GB on C: despite
npm being redirected. Verified after the change: `npx yarn` writes to D: and does not recreate the
C: dirs. Per-command `npm install --cache <dir>` also works but does NOT cover yarn.
