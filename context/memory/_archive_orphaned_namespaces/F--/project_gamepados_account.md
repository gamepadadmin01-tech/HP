...
description: "GamepadOS Account System rebuild in F:\\App with login — repo, architecture decisions, phase plan"
metadata: 
  node_type: memory
  type: project
...
🚩 **ALL WORK HAPPENS IN `F:\hlooo`** (Akhil's call, 2026-07-24 — "everything preserved,
one tree"). `F:\App with login` was a partial fork holding Phases 0–1; that work is now
ported into hlooo and the fork is **abandoned** — do not edit it.
- App source (`F:\hlooo\apps\**`): has **no version control**. Backups only, e.g.
  `App.tsx.bak-20260722-preaccount`. Take one before every App.tsx edit.
- Backend (`F:\hlooo\website` = repo `gamepadadmin01-tech/gamepad`): **make changes,
...
**Phase 4 backend WRITTEN 2026-07-24, UNCOMMITTED in `F:\hlooo\website`** (Akhil commits +
