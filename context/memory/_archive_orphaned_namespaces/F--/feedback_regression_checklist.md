...
* **`F:\hlooo\docs\REGRESSION_CHECKLIST.md`** — the bug register. Each entry is a
...
* **`F:\hlooo\tools\regression-check.sh`** — runs the automatable half:
...
`F:\hlooo\apps\` is **not a git repo** — timestamped `.bak-` copies are the only
